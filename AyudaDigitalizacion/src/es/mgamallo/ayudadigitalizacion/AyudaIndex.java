...
public class AyudaIndex {
	...
	public AyudaIndex(boolean editar, boolean confirmar) {
	...
	//		Txt.escribirIndiceTxt(indiceGeneral);
			...
			if(Txt.escribirIndiceTxt(indiceGeneral)){
				if(confirmar)
					JOptionPane.showMessageDialog(null, "Fichero hermes.txt indexado.");
			}
			else{
			...
			while(it.hasNext()){
			  ...
			  Inicio.comboModelMetadatos.addElement(key);
			}
			
			String tabla[][] = Inicio.leerExcel.getTablaHermesAyuda();
			Set<String> nombres = new TreeSet<String>();
			for(int i=0;i<tabla.length;i++){
				nombres.add(tabla[i][1]);
			}
			
			Inicio.comboModelNombresExistentes.removeAllElements();
			
			it = nombres.iterator();
			while(it.hasNext()){
				Inicio.comboModelNombresExistentes.addElement(it.next());
			}
